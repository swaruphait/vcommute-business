...
app.controller("UserViews", function ($scope, $http, $timeout) {
	...
	$scope.fetchApprovalLevel = function (str) {
		if (str==null) {
            str = 0;
        }
		$http({
	...
	$scope.fetchGrade = function (str) {
		...
		$http({
			...
			$scope.appvlLevel = response.data.data;
		}, function errorCallback(response) {
			console.log(response.statusText);
		});
	};
	...
	$scope.editUserById = function (str) {
		$http({
			...
			$scope.fetchRoles($scope.form.companyId);
			$scope.fetchDepartment($scope.form.companyId);
			$scope.fetchApprovalLevel($scope.form.companyId);
			$scope.fetchGrade($scope.form.companyId);
